...
function loadData(){try{if(fs.existsSync(DATA_FILE))return JSON.parse(fs.readFileSync(DATA_FILE,'utf8'))}catch(e){}return{conversations:[],lastUpdated:null,lastRawPayload:null};}
function saveData(d){d.lastUpdated=new Date().toISOString();fs.writeFileSync(DATA_FILE,JSON.stringify(d,null,2));}
var db=loadData();
function getKey(keys,name){var k=keys.find(function(x){return x.key===name;});return k?k.value:null;}
function getText(m){if(m.text&&typeof m.text==='string')return m.text;if(m.text&&m.text.message)return m.text.message;if(m.hsm&&m.hsm.message)return m.hsm.message;return '';}
app.post('/webhook/treble',function(req,res){
  ...
  var tr=0,rc=0;
  for(var i=1;i<msgs.length;i++){var a=msgs[i-1],b=msgs[i];if(esU(a)&&esA(b)&&a.created_at&&b.created_at){var d=(new Date(b.created_at)-new Date(a.created_at))/60000;if(d>0&&d<120){tr+=d;rc++;}}}
  var keys=p.user_session_keys||[];
  var pm=ma[0];
  // Buscar agente: primero en session_keys (owner, owner_email), luego en mensajes
  var aN=getKey(keys,'owner')||getKey(keys,'agent_name')||getKey(keys,'vendedor')||getKey(keys,'asesor')||(pm&&pm.agent_name)||(pm&&pm.name)||'Automatico';
  var aI=getKey(keys,'owner_email')||getKey(keys,'agent_id')||(pm&&pm.agent_id)||aN;
  var txt=msgs.map(function(m){return getText(m);}).join(' ').toLowerCase();
  var tmap={Precio:['precio','costo','cuanto','descuento','oferta','promo','barato','caro','valor','paquete','combo'],Soporte:['error','falla','problema','no funciona','ayuda','soporte'],Envio:['envio','entrega','llega','pedido','despacho'],Devolucion:['devolver','reembolso','cambio','garantia'],Pago:['pago','pagar','tarjeta','efectivo','factura','transferencia'],Ventas:['ventas','vende','restaurante','activa','invierte','rappi','ads','publicidad','ejecutivo','comercial'],Saludo:['hola','buenos','gracias','bienvenido','bye','disculpame']};
  var temas=Object.keys(tmap).filter(function(t){return tmap[t].some(function(k){return txt.indexOf(k)>-1;});});
  var mc=ma.filter(function(m){var t=getText(m);return t&&t.length<20;}).length;
  var rc2=ma.length>0?mc/ma.length:0,ti=rc>0?tr/rc:999;
  var sc=100;if(msgs.length<3)sc-=30;if(ma.length===0)sc-=40;if(ti>60)sc-=20;else if(ti>20)sc-=10;if(rc2>0.7)sc-=25;else if(rc2>0.4)sc-=10;sc=Math.max(0,sc);
  var ini=(msgs[0]&&msgs[0].created_at)||(p.session&&p.session.closed_at),fin=p.session&&p.session.closed_at;
  db.conversations.push({eventId:p.event_id,sessionId:p.session&&p.session.external_id,cerradoEl:p.session&&p.session.closed_at,recibidoEl:new Date().toISOString(),hora:ini?new Date(ini).getHours():null,diaSemana:ini?new Date(ini).getDay():null,fecha:ini?new Date(ini).toISOString().split('T')[0]:null,agenteId:aI,agenteName:aN,telefonoUsuario:((p.user&&p.user.country_code)||'')+((p.user&&p.user.cellphone)||''),totalMensajes:msgs.length,msgsAgente:ma.length,msgsUsuario:mu.length,tiempoRespMin:rc>0?Math.round(tr/rc*10)/10:null,duracionMin:ini&&fin?Math.round((new Date(fin)-new Date(ini))/60000):0,score:sc,temas:temas,textoResumen:txt.substring(0,500),fraude:{convMuyCorta:msgs.length<3,sinRespuestaAgente:ma.length===0,msgsDemasiadoCortos:rc2>0.8,respuestaMuyLenta:rc>0&&ti>60}});
  saveData(db);
  console.log('[OK] Agente:'+aN+' ('+aI+') msgs:'+msgs.length+' temas:'+temas.join(','));
});
